...
class listener_info_t(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(listener_info_t._get_packed_fingerprint())
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != listener_info_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return listener_info_t._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = listener_info_t()
    ...
    def _get_hash_recursive(parents):
        if listener_info_t in parents: return 0
    ...
    def _get_packed_fingerprint():
        if listener_info_t._packed_fingerprint is None:
            listener_info_t._packed_fingerprint = struct.pack(">Q", listener_info_t._get_hash_recursive([]))
        return listener_info_t._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", listener_info_t._get_packed_fingerprint())[0]
