...
def twist(linear_velocity: np.ndarray, angular_velocity: np.ndarray) -> twist_t:
    ...
    if linear_velocity.shape != (3,):
        raise ValueError(f"linear_velocity must be numpy array of shape (3,), got {linear_velocity.shape}")
    if angular_velocity.shape != (3,):
        raise ValueError(f"angular_velocity must be numpy array of shape (3,), got {angular_velocity.shape}")
...
def robot_init(name: str, position: np.ndarray, orientation: np.ndarray, q_init: np.ndarray) -> robot_init_t:
    ...
    if q_init.ndim != 1:
        raise ValueError(f"q_init must be a 1D numpy array, got shape {q_init.shape}")
...
def rigid_body_state(
    ...
    if lin_velocity.shape != (3,):
        raise ValueError(f"lin_velocity must have shape (3,), got {lin_velocity.shape}")
    if ang_velocity.shape != (3,):
        raise ValueError(f"ang_velocity must have shape (3,), got {ang_velocity.shape}")
...
def force(name: List[str], force: np.ndarray) -> force_t:
    ...
    """
    assert len(name) == len(force), "Force arrays must match the same of the names"
    msg = force_t()
    msg.n = len(name)
    msg.name = name
    msg.force = force
...
def pose(position: np.ndarray, orientation: np.ndarray) -> pose_t:
    ...
    """
    if position.shape != (3,):
        raise ValueError(f"Position must be a numpy array with shape (3,), got {position.shape}")
    if orientation.shape != (4,):
        raise ValueError(f"Orientation must be a numpy array with shape (4,), got {orientation.shape}")
...
def imu(orientation: np.ndarray, gyro: np.ndarray, accel: np.ndarray) -> imu_t:
    ...
    if orientation.shape != (4,):
        raise ValueError(f"orientation must be a numpy array with shape (4,), got {orientation.shape}")
    if gyro.shape != (3,):
        raise ValueError(f"gyro must be a numpy array with shape (3,), got {gyro.shape}")
    if accel.shape != (3,):
        raise ValueError(f"accel must be a numpy array with shape (3,), got {accel.shape}")
